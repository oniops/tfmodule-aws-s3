locals {
  attach_policy = var.attach_deny_insecure_transport_policy || var.attach_custom_policy
...
data "aws_iam_policy_document" "pols" {
  ...
  source_policy_documents = compact([
    var.attach_deny_insecure_transport_policy ? data.aws_iam_policy_document.denyInsecureTransport[0].json : "",
